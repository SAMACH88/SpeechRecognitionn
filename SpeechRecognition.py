...
def transcribe_speech(api_choice, language, audio_data=None):
    try:
        if audio_data is None:  # If no uploaded audio, use microphone
            with sr.Microphone() as source:
                st.write("Listening...")
                audio = recognizer.listen(source)
        else:  # Use uploaded audio
...
def main():
    ...
    device_message = check_audio_input_device()
    st.write(device_message)
    ...
    if st.button("Start Listening") or audio_file:
        if audio_file:
            transcribed_text = transcribe_speech(APIS[api_choice], selected_language, audio_file)
        else:
